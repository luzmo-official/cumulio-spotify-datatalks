import {CUMULIO_PLAYLIST} from './app.js';
...
class UI {
  ...
  async generatePlaylistSongList(playlist) {
    ...
    visualizeButtonEl.onclick = () =>   window.openPageCumulioFavorites();
    visualizeButtonEl.innerText = 'Visualize';
    if(playlist.id === CUMULIO_PLAYLIST) containerEl.append(visualizeButtonEl);
    ...
    playlists.forEach(playlist => {
      ...
      div.onclick = async () => {
        ...
        if (response.snapshot_id !== 'undefined') {
          ...
          t.succesfullyAddedToPlaylist(true, name, id, playlist);
        }
        else {
          t.succesfullyAddedToPlaylist(false, name, id, playlist);
        }
      };
      div.innerHTML = `
  ...
  succesfullyAddedToPlaylist(success, song, id, playlist){
    const t = this;
    ...
    modalTitle.innerText = success ? 'Succes' : 'Sorry, problem encountered :(';
    ...
    ` :`
    <div id="success-info">
      <div>Encountered problems while adding <b><i>${song}</i></b> to <b>${playlist.name}</b>!</div>
      <button id="add-song-again-btn" type="button" class="btn btn-primary">Add to other playlist</button>
    </div>
    `;
